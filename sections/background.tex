...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[auto, node distance=8cm,>=latex']
        \node[coordinate, name=start](start){};
        \node[block, right of=start](ptd){Problem (Time Domain)};
        \node[block, right of=ptd](std){Solution (Time Domain)};
        \draw [->] (ptd) -- node[name=u] {Convolution} (std);
        \node[block, name=psd, below of=ptd](psd){Problem ($s$ Domain)};
        \node[block, right of=psd](ssd){Solution ($s$ Domain)};
        \draw [->] (ptd) -- node[node distance=2cm]{$\mathcal{L}$ Table} (psd);
        \draw [->] (psd) -- node {TF Algebra} (ssd);
        \draw [->] (ssd) -- node {$\mathcal{L}^{-1}$ Table} (std);
    \end{tikzpicture}
...
\begin{figure}[H]
    \centering
    % \includegraphics[width=0.5\linewidth]{MSD.png}
    \begin{tikzpicture}
        % \node[draw,outer sep=0pt,thick] (M2) at (3,0) [minimum width=1.5cm, minimum height=1.5cm] {$m_2$};
        \node[ground] (wall) [minimum width=0.1cm, minimum height=2cm] {};
        \node[draw,outer sep=0pt,thick] (M1) at (3,0) [minimum width=1.5cm, minimum height=1.5cm] {$m$};
        \node[ground] (ground) at (1.5, -0.9) [minimum width=5cm, minimum height=0.1cm] {};
        \draw[spring] ($(wall.east) - (0,0.5)$) -- ($(M1.west) - (0,0.5)$) node [midway,above] {$k$};
        \draw[damper] ($(wall.east) + (0,0.5)$) -- ($(M1.west) + (0,0.5)$) node [midway,above] {$c$};
        % \draw[thick, dashed] ($(M1.north west)$) -- ($(M1.north west) + (0,1)$);
        % \draw[thick, dashed] ($(M2.north west)$) -- ($(M2.north west) + (0,1)$);
        \draw[ultra thick, -latex] ($(M1.east)$) -- ($(M1.east) + (1,0)$) node [midway, below] {$F$};
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{RLC.png}
    \caption{RLC System}
    \label{fig:RLC}
\end{figure}
